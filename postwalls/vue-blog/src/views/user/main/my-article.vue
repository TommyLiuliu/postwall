<template>
    <div class="my-article">
        <el-breadcrumb separator="/" style="margin-bottom: 20px">
            <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
            <el-breadcrumb-item>我的文章</el-breadcrumb-item>
        </el-breadcrumb>
        <el-button type="primary" @click="handleAdd">
            <i class="fa fa-pencil"></i> 发表文章
        </el-button>
        <div class="articles">
            <a href="javascript: void(0)" v-for="(item, index) in articles" :key="index" >
                <el-card class="box-card" shadow="hover">
                    <div class="category">
                        {{item.categoryName}}
                    </div>
                    <div class="cover" @click="$router.push('/articleView/' + item.link)">
                        <el-image style="width: 380px; height: 254px" :src="item.coverUrl" fit="cover" />
                    </div>
                    <div class="article-info">
                        <h2 class="title" @click="$router.push('/articleView/' + item.link)">{{item.title}}</h2>
                        <div class="meta">
                            <span><i class="fa fa-clock-o"></i> {{item.publishDate}}</span>
                            <span><h4 style="display: inline-block;margin-right: 5px" v-for="(tag, tagIndex) in item.tags"
                                      :key="tagIndex"><i class="fa fa-tag"></i>{{tag}} </h4></span>
                            <el-button type="primary" @click="handleEdit(item)">编辑</el-button>
                            <el-button type="danger" @click="handleDelete(item)">删除</el-button>
                        </div>
                    </div>
                </el-card>
            </a>
        </div>
        <el-empty v-show="articles.length == 0" description="没有文章哦，请发表文章吧~"/>
        <el-pagination
                background
                :current-page="page.curPage"
                :page-size="page.pageSize"
                layout="total, prev, pager, next"
                :total="page.total"
                @size-change="handleChangeSize"
                @current-change="handleChangePage"
        ></el-pagination>
        <article-edit ref="edit" @reloadTable="reloadTable"/>
    </div>
</template>

<script>
    import { myArticles, deleteArticle } from "@/api/user/article";
    import articleEdit from "./article-edit"
    export default {
        name: "my-article",
        components: {
            articleEdit
        },
        data() {
          return {
              searchForm: {
                  title: ''
              },
              articles: [],
              page: {
                  curPage: 1,
                  pageSize: 10,
                  total: 0,
              },
          }
        },
        created() {
            this.myArticles();
        },
        methods: {
            myArticles() {
                this.searchForm.curPage = this.page.curPage
                this.searchForm.pageSize = this.page.pageSize
                myArticles(this.searchForm).then((res) => {
                    if (res.code == 200) {
                        this.articles = res.data.articles
                        this.page.total = res.data.count
                    }
                })
            },
            handleChangePage(number) {
                this.page.curPage = number;
                this.myArticles();
            },
            handleChangeSize(number) {
                this.page.pageSize = number;
                this.myArticles();
            },
            handleAdd() {
                this.$refs['edit'].showEdit()
            },
            handleEdit(row) {
                this.$refs['edit'].showEdit(row)
            },
            reloadTable() {
                this.searchForm = this.$options.data().searchForm;
                this.page.curPage = 1;
                this.myArticles();
            },
            handleDelete(row) {
                this.$confirm("你确定要删除文章【"+row.title+"】？", "提示", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning",
                    "close-on-click-modal": false,
                }).then(() => {
                    deleteArticle(row.id).then((res) => {
                        if (res.code == 200) {
                            this.reloadTable()
                            this.$message({ type: "success", message: res.message });
                        } else {
                            this.$message({ type: "error", message: res.message });
                        }
                    })
                })
            },
        }
    }
</script>

<style scoped>
    .articles {
        margin-top: 20px;
    }
    .cover {
        display: inline-block;
    }
    .article-info {
        display: inline-block;
        vertical-align: top;
        margin-left: 15px;
        max-width: 252px;
    }
    .title {
        overflow: hidden;
        text-overflow: ellipsis;
        display: -webkit-box;
        -webkit-line-clamp: 3;
        -webkit-box-orient: vertical;
        height: 81px;
    }
    .username {
        vertical-align: top;
        font-size: 14px;
        font-weight: bold;
        line-height: 40px;
        margin: 0 10px;
    }
    .category {
        position: absolute;
        margin-left: -16px;
        margin-top: 14px;
        background-color: #fdbc40;
        box-shadow: 0 1px 5px #fdbc40;
        letter-spacing: 4px;
        z-index: 10;
        display: inline;
        padding: 0.8rem;
        font-size: 75%;
        font-weight: 700;
        line-height: 1;
        color: #fff;
        text-align: center;
        white-space: nowrap;
        vertical-align: baseline;
        border-radius: 0.25em;
    }
    .category:before {
        content: "";
        position: absolute;
        top: 100%;
        left: 0;
        width: 0;
        height: 0;
        background-color: transparent;
        border-style: solid;
        border-width: 0 1rem 1rem 0;
        border-color: transparent;
        border-right-color: #fdbc40;
        filter: brightness(.9);
    }
    .meta {
        margin: 15px 0;
    }
    .meta span {
        display: block;
        margin: 5px 0;
    }
    .articles .el-card {
        margin-bottom: 20px;
    }
</style>
